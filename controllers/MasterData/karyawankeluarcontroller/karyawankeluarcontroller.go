package karyawankeluarcontroller
...
func Index(c *fiber.Ctx) error {
	var data []models.KaryawanKeluar
...
func Show(c *fiber.Ctx) error {
	IdKeluar := c.Params("IdKeluar")
	var data models.KaryawanKeluar

	if err := DB.First(&data, "IdKeluar = ?", IdKeluar).Error; err != nil {
...
func Create(c *fiber.Ctx) error {
	data := new(models.KaryawanKeluar)
...
func Update(c *fiber.Ctx) error {
	...
	if err := DB.First(&models.KaryawanKeluar{}, "IdKeluar = ?", IdKeluar).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return respond(c, http.StatusNotFound, "Data tidak ditemukan", nil, 0)
		}
		return respond(c, http.StatusInternalServerError, "Gagal mengambil data", nil, 0)
	...
	idUint, _ := strconv.ParseUint(IdKeluar, 10, 64)
	data.IdKeluar = uint(idUint)
...
func Delete(c *fiber.Ctx) error {
	...
	if err := DB.Delete(&models.KaryawanKeluar{}, "IdKeluar = ?", IdKeluar).Error; err != nil {
